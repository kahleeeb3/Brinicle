...
https://repository.up.ac.za/bitstream/handle/2263/60776/Anguelov_Stationary_2017.pdf;jsessionid=E7480271C08863EB2E9AF2DDA89C6273?sequence=1

The reaction implemented here is the Brusselator
For convenience the two species x and y are renamed u and v
such that the reactions are
a -> u
2u + v -> 3u
b + u -> v + d
u -> e
where e and d are end products which can be ignored and a and b are constant.
...
#    vpxy = np.vstack((vpy[1,:], vpy, vpy[-2,:]))
    ...
    up=up+(uxx+uyy+uxy+uyx)+(uxvel+uyvel) + dt*(a + (up**2)*vp - b*up - up)
    vp=vp+(vxx+vyy+vxy+vyx)+(vxvel+vyvel) + dt*(b*up - (up**2)*vp)
...
Lx = 100         # This sets the x length of the simulation in mm.
...
a = 1           # This is the parameter a from the Brusselator.
b = 3           # This is the parameter b from the Brusselator.
...
"""
u_amp = u0/2      # Set to zero to start everything the same
v_amp = v0/2      # Set to zero to start everything the same
...
while telap <= tEnd:    
    ...
    fname = 'Brusselator/Patterns_%06d.png' % t # Create the file name for each plot
