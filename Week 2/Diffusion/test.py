...
Lx = 15        # This sets the x length of the simulation in mm.
...
"""
Use this section to define the shape we would like
"""

# This is the middle spot of specified width
spotwidth=30 # This is half width
spotleft=int(np.round(nx/2))-spotwidth+70 # Determine the left edge
spotright=int(np.round(nx/2))+spotwidth+70 # Determine the right edge
...
while telap <= tEnd:    
    ...
    if displayUandV: # If the Boolean is set to plot both u and v
        ...
        # Create the filled countour plot with colormap and manual levels
        cf = ax.contourf(X,Y,Z,cmap=cm.gist_heat,levels=np.arange(0,260,0.5))
