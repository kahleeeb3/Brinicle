...
CHANGES:
    1) Different freezing temperatures based on salinity (Ice Function)
    2) Advection Back to the partial diffusion model. (Chemical Parameters)
    3) Initial Conditions now match researched values (Initial Conditions)
    
TODO:
    4) Diffusion Rate of salt depends on the temperature
...
res = 0.25   #0.5    # This sets the resolution for the simulation in mm/step.
...
dt = 0.025   #0.05   # The time step for the calculation in s.
...
t = 0#tEnd  # Set this value to 0 to make all plots
...
if t == 0:
    ...
    while telap <= tEnd:  
        ...
        functions.plot(folder,tEnd, Lx, Ly, res, data, l, dt, dtWindow, telap)
    ...
    functions.plot(folder,tEnd, Lx, Ly, res, data, l, dt, dtWindow, t)
